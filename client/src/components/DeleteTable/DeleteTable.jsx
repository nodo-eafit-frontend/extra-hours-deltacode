...
export const DeleteTable = () => {
  ...
  const deleteRecord = async (record) => {
    try {
      await axios.delete(`http://localhost:4000/extra-hour/${record.registry}`);
      message.succes("Record delete successfully!");

      setRecords((prevRecords) =>
        prevRecords.filter((item) => item.registry !== record.registry)
      );
    } catch (error) {
